...
def scrape(since=None, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
    ...
    with open(path, write_mode, newline='', encoding='utf-8') as f:
        ...
        while until_local <= datetime.datetime.strptime(until, '%Y-%m-%d'):
            ...
            scrolling = True
            print("looking for tweets between " + str(since) +
                  " and " + str(until_local) + " ...")
    ...
    driver.close()
    return data
# retrived_data=scrape(words='regime',from_account="ImranKhanPTI",until='2022-07-15',since='2022-07-10',limit=10, interval=1,display_type='Latest',lang="en", headless=True)


######################            Twitter Space Cleaning ###############################


def pre_clean_df_spaces(df):
    indexs_of_avaliable_spaces = []
    df['splink'] = ''
    for i in range(len(df)):
        lnk = df['Embedded_text'][i]
        try:
            temp = re.search("(?P<url>https?://[^\s]+)", lnk).group("url")
            df['splink'][i] = temp[:-1]
        except:
            indexs_of_avaliable_spaces.append(i)
    df = df.drop(indexs_of_avaliable_spaces, axis=0)
    return df
